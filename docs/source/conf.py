...
# aio-pika 文档构建配置文件，由 sphinx-quickstart 创建于 2017 年 3 月 31 日 17:03:20。

# 该文件使用 execfile() 方式执行，当前目录设置为其包含目录。

# 请注意，并不是所有可能的配置值都包含在这个自动生成的文件中。

# 所有配置值都有默认值；被注释掉的值用于显示默认值。

# 如果扩展（或要通过 autodoc 文档化的模块）在其他目录中，请在此处将这些目录添加到 sys.path。如果目录相对于文档根目录，则使用 os.path.abspath 将其转换为绝对路径，如这里所示。
